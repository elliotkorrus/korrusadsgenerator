...
async function seed() {
  ...
  const angles = [
    ...
    { angleSlug: "value-prop", description: "Highlight unique product benefits", example: "One bulb. Zero blue light. Better sleep.", product: "OIO", funnelStage: "MOF", status: "active" },
    { angleSlug: "offer", description: "Promote deals, bundles, limited-time pricing", example: "Save 20% this week — starter kit from $29", product: "OIO", funnelStage: "BOF", status: "active" },
    ...
    { angleSlug: "authority", description: "Expert endorsements, press, credentials", example: "Recommended by Dr. Huberman's sleep protocol", product: "OIO", funnelStage: "MOF", status: "active" },
  ...
  const existingAngles = await db.select().from(schema.angleBank);
  ...
  const existingFields = await db.select().from(schema.fieldOptions);
  ...
  const existingCopy = await db.select().from(schema.copyLibrary);
